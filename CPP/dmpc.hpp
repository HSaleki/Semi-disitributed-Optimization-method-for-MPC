...
// the null space of its active constraints and returns only the Schur block
//
...
// so the warm start is exactly optimal at sigma = 1, then the perturbation is
// retired linearly. sigma is the state variable (not tau = 1 - sigma), so the
// right-hand sides come back bitwise at sigma = 0.
//
// C++17, Eigen 3 only. Port of the MATLAB/Python implementation.
...
using Mask = std::vector<char>;      // std::vector<bool> has no data(); avoid it
...
// ||v||_inf with the empty vector mapping to 0 rather than asserting
...
// Orthonormal basis of ker(A). tol is relative to the largest singular value.
...
// Columns are processed in the order given: a column dependent on the ones
// before it is skipped, never reordered. Warm-start order therefore decides
// which constraint of a dependent group is dropped.
...
// Z'QZ, factored once per active-set change.
class Reduced {
  public:
...
class LocalNode {
  public:
    ...
    // flag: 1 = this node is limiting, 0 = it is not, -1 = terminal signal
    void receiveDeltaTau(double dtau, int flag, const Mask& actCnew) {
        ...
        if (flag == 1) {
            ...
            // is not if/else -- otherwise mu(k_d) keeps a -ulp residual
  ...
  private:
    ...
    // Drop rows until Hz(W,:) has full row rank. The leaving row is picked by a
    // dual ratio test on the dependency null vector, so H'mu + G'lam is left
    // unchanged and mu >= 0 survives -- d_p = p_0 - p is not perturbed.
    std::vector<int> resolveDependency() {
        ...
        if (!dropped.empty() && neq_) {
            // Hz(W,:)'(mu - mu0) = 0 => H'(mu - mu0) in range(G'), so this
            // compensation is exact, not least squares
    ...
    // zero curvature after release means the QP is unbounded along pdir
...
inline Solution solve(const Problem& prob, Logger& log, int maxIters = 10000) {
    ...
    // do not hand back a non-converged point that looks like a solution
