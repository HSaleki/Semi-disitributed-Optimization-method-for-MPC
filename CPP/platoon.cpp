// Closed-loop platoon MPC example.
// z_i = [u0; x1; u1; ...; u_{N-1}; xN], x = [p; v; a].
// Consecutive vehicles satisfy the minimum-gap constraint.
...
// Discrete-time LQR from the Riccati recursion
...
// Shift the previous solution. The last input is filled with the clipped LQR
// control and its active bound is kept for the dual warm start.
void shiftZ(const Mat& Ad, const Mat& Bd, const std::vector<Vec>& Zprev,
    ...
    for (int i = 0; i < M; ++i) {
        ...
        if (kr >= 0) {
            actN[i] = (kr + 1) * N - 1;          // last row of block kr
...
// Shift each block and keep its last entry.
template <typename T>
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (s == "-h" || s == "--help") {
            std::printf(
                ...
                "               [--csv PATH] [--quiet]\n"
                "\n"
                "  steps      closed-loop steps to simulate (default 10)\n"
        ...
        if (!Zprev.empty()) {
            ...
            for (int i = 0; i < M; ++i) {
                for (int blk = 1; blk <= 6; ++blk) {
                    const int row = blk * N - 1;   // last row of each block
                    locActive[i][row] = 0;         // shiftZ handles the last row
