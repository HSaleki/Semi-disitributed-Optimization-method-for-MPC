...
for i = 1:N
    nodes{i} = LocalNode(i, Q{i}, p{i}, G{i}, g{i}, H{i}, hl{i}, hu{i}, A{i}, Aflag, tolerance);
end

% Step 4: Initialize Central Node
central = CentralNode(nodes, cell2mat(lb), cell2mat(ub), tolerance);

% Step 5: Optimization Loop
while iter <= max_iters 

    % Receive matrices and vectors form local nodes and compute delta_mu
    central.aggregateAndSolveDeltaz();

    % Receive matrices and vector to form merit function and determine step
    central.aggregateAndSolvestep();

    exit_flag = central.barrierstep();
    if exit_flag == 1
